...
function extractSqlBlocks(content: string): string | null {
...
function SegmentTabs({
  ...
  return (
    <div className="my-3" ref={containerRef}>
      ...
      {activeView === "raw" && (
        <div className="overflow-x-auto my-1 space-y-3">
          {sqlBlock && (
            <div>
              <p className="text-[10px] font-semibold text-emerald-700 dark:text-emerald-400 mb-1">Consulta SQL:</p>
              <pre className="bg-slate-800 dark:bg-zinc-900 rounded-md p-3 text-[11px] font-mono text-emerald-300 whitespace-pre-wrap leading-relaxed border border-slate-700">
                {sqlBlock}
              </pre>
            </div>
          )}
          <div>
            <p className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 mb-1">Datos crudos:</p>
...
export function MessageContent({
  ...
  const sqlBlock = useMemo(() => {
    if (isStreaming || !safeContent) return null;
